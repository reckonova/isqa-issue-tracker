...
module.exports = function (app) {
  ...
  app.route('/api/issues/:project')
    ...
    .put(function (req, res){
      ...
      Issue.findOneAndUpdate({_id: updateData._id}, {$set: updateData}, {new: true}, function(error, data){
        if(error){ res.json({failure: 'could not update '+data._id}); return;}
    ...
    .delete(function (req, res){
      ...
      Issue.findById({_id: idDelete}, function(err, data){
        if(err) {res.json({error: '_id error'}); return;}
        //check to see if id exists if it does delete it
        if(!data){res.json({error: '_id error'}); return;}
        ...
        Issue.findByIdAndRemove({_id: idDelete}, function(err, data){
          ...
          //checks to see if it exists after deleting it     
            ...
            Issue.findById({_id: idDelete}, function(err, data){
              ...
              if(data){ res.json({failed: 'could not delete '+idDelete});}
              else{ res.json({sucess: 'deleted '+idDelete});}
            });
          }
